# Units & measurement rules

These rules apply everywhere: recipes, journal entries, cook-alongs, shopping lists, and casual chat.

## Mass: always grams first
...
- **Aromatics and herbs:** grams first, with the count or size as the helper: `12g garlic (3 large cloves), minced`, `225g yellow onion (1 medium), diced`.
- **"To taste"** is only for final adjustments, and always comes with a starting point: `black pepper, to taste (start ~1g)`.
- **Converting volume recipes:** use trusted density data. For baking staples that's King Arthur's ingredient weight chart. Otherwise use manufacturer or USDA data. Mark estimates with `~`. Weights we've measured ourselves (recorded in recipes or techniques) take priority over everything.

## Ratios

- **Bread, dough, and pastry** include a baker's percentage table (flour = 100%), with hydration and salt %.
- **Brines, cures, and seasoning by weight** give the percentage and the grams: `dry brine at 1% (9g salt per 900g meat)`.
...
- `½" (1cm) dice`, `12" (30cm) skillet`. Give pan diameters as the manufacturer sells them. Give capacities as sold, with a liters helper: `5.5qt (5.2L)`.
...
- Describe heat by what it does, and also by Brian's own dial setting once it's been calibrated (see `kitchen/equipment.md` → Range): `medium-high (7/10 on the big burner): butter foams but doesn't brown`.

## Fractions

- In helper text, use Unicode fractions: ½ ⅓ ⅔ ¼ ¾ ⅛.
